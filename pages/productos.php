<?php
    ...
    function mostrarServicios_emprendedor(){
        ...
        $qry_servicios = "SELECT S.idproductos, S.nombre, S.descripcion, S.precio, S.img_producto, S.estatus, E.telefono, C.nombre AS categoria, S.fk_idemprendedor, S.estatus
                            FROM productos S
                            LEFT JOIN emprendedor E ON E.idemprendedor = S.fk_idemprendedor
                            LEFT JOIN usuarios U ON U.idusuarios = E.fk_idusuarios
                            LEFT JOIN categoria_productos C ON C.idcategoria_productos = S.fk_idcategoria_productos
                            WHERE E.fk_idusuarios = $idusuario
                            AND (S.estatus= 'V' OR S.estatus= 'NV')";

        $res_servicios = ejecutar_sql($qry_servicios);
        $tabla = '';
        $tabla.= '<h1 class="font-bold" style="--tw-text-opacity: 1; color: rgb(255 80 0 / var(--tw-text-opacity));">Tus productos:</h1>';
        ...
        if ($res_servicios->num_rows > 0){
            while ($row = $res_servicios->fetch_assoc()){
                ...
                $tabla.= '<a style="cursor: pointer;"  href="./editar_producto.php?id=' . $row["idproductos"] . '" class="bg-azul text-blanco font-semibold py-2 px-4 rounded-md hover:bg-blue-900">Editar</a>';
            ...
            $tabla= '<h1 class="font-bold" style="--tw-text-opacity: 1; color: rgb(255 80 0 / var(--tw-text-opacity));">Tus productos:</h1>';
            $tabla.= '<h1 class="font-bold" style="--tw-text-opacity: 1; color: rgb(255 80 0 / var(--tw-text-opacity));">No tienes productos registrados</h1>';
        }
        return $tabla;
    }

    function mostrarServicios_estudiante($categoria){
        $qry_servicios = "SELECT S.idproductos, S.nombre, S.descripcion, S.precio, S.img_producto, S.estatus, E.telefono, C.nombre AS categoria, S.fk_idemprendedor
                            FROM productos S
                            LEFT JOIN emprendedor E ON E.idemprendedor = S.fk_idemprendedor
                            LEFT JOIN usuarios U ON U.idusuarios = E.fk_idusuarios
                            LEFT JOIN categoria_productos C ON C.idcategoria_productos = S.fk_idcategoria_productos
                            WHERE C.idcategoria_productos = $categoria
                            AND S.estatus= 'V'";

        $res_servicios = ejecutar_sql($qry_servicios);
        $qry_cat_serv = "SELECT * FROM categoria_productos WHERE idcategoria_productos = $categoria";
        ...
        if ($res_servicios->num_rows > 0){
            while ($row = $res_servicios->fetch_assoc()){
                ...
                $tabla.= '<a style="cursor: pointer;"  href="./ver_producto.php?id=' . $row["idproductos"] . '" class="bg-azul text-blanco font-semibold py-2 px-4 rounded-md hover:bg-blue-900">Ver Más</a>';
                $tabla.= '<a style="cursor: pointer;"  href="./realizar_pedido.php?id=' . $row["idproductos"] . '" class="bg-azul text-blanco font-semibold py-2 px-4 rounded-md hover:bg-blue-900">Realizar pedido</a>';
                $tabla.= '</div>';
                $tabla .='<br>';
                $tabla.= '</div>';
                $tabla.= '</div>';
...
?>
            <section class ="w-full h-full flex flex-col justify-center items-center">
                ...
                if ($_SESSION['usuario_tipo'] == "EST"){
                    $qry_categorias_servicios = "SELECT * FROM categoria_productos";
                    $res_categorias_servicios = ejecutar_sql($qry_categorias_servicios);
                    echo '<h1 class="font-bold" style="--tw-text-opacity: 1; color: rgb(255 80 0 / var(--tw-text-opacity));">Productos disponibles</h1>';
                    while($row = $res_categorias_servicios->fetch_array()){
                        $categoria_servicio = $row["idcategoria_productos"];
